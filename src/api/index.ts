...
export const devicesAPI = {
  list: async (params?: { page?: number; page_size?: number; search?: string; status?: string; group_id?: string; enrollment_token?: string; issam_search?: string; issam_filter?: string; last_seen_from?: string; last_seen_to?: string; agent_version?: string }): Promise<DeviceListResponse> => {
...
export const commandsAPI = {
  ...
  captureIssam: async (deviceId: string): Promise<Command> => {
    const response = await api.post<APIResponse<Command>>(`/devices/${deviceId}/capture-issam`)
    return response.data.data!
  },
  sendTestNotification: async (deviceId: string, issamId: string): Promise<Command> => {
